...
st.caption("Datos públicos EIA | Clasificación supervisada | Predicción del siguiente periodo | Contraste con compras reales de importación")
...
def clasificar_correlacion(corr: float) -> str:
    """Clasifica la intensidad de la relación lineal observada."""
    if pd.isna(corr):
        return "Sin dato"
    abs_corr = abs(corr)
    if abs_corr >= 0.80:
...
def texto_transmision_precios(backtest: pd.DataFrame, etiqueta_eia: str) -> str:
    """Lectura económica de la relación entre componente internacional y compra real."""
    corr = calcular_correlacion(backtest)
    nivel = clasificar_correlacion(corr)
    if pd.isna(corr):
        return (
            "No hay observaciones suficientes para estimar una correlación histórica entre el proxy EIA "
            "y las compras reales de importación."
        )

    return (
        f"La correlación histórica entre el proxy internacional ({etiqueta_eia}) y el precio real de compra Recope "
        f"es de {corr:,.2f}, clasificada como {nivel.lower()}. Esto sugiere que el componente externo sí se mueve "
        "en la misma dirección general que las compras reales, aunque no necesariamente con la misma magnitud. "
        "En términos económicos, el gráfico permite observar una transmisión parcial de precios internacionales: "
        "las compras reales tienden a seguir la tendencia del mercado externo, pero con posibles rezagos, "
        "amortiguación logística, condiciones contractuales y diferencias propias del costo de importación."
    )


def mostrar_glosario_metricas():
    """Muestra definiciones técnicas breves para facilitar lectura del panel."""
    with st.expander("Glosario técnico de esta sección"):
        st.markdown(
            """
            **MAE (Mean Absolute Error):** promedio absoluto de las diferencias entre el valor estimado y el valor real. 
            En esta app se expresa en USD/bbl. Mientras menor sea, más cercano está el proxy al precio real.

            **MAPE (Mean Absolute Percentage Error):** error porcentual absoluto promedio. Permite leer el error en términos relativos. 
            Un MAPE de 10% indica que, en promedio, la desviación fue cercana al 10% del precio real.

            **Cercanía:** indicador construido como `100 - MAPE` o `100 - error porcentual absoluto` para cada observación. 
            No es una métrica estadística estándar, sino una forma sencilla de comunicar proximidad.

            **Accuracy:** porcentaje de aciertos del modelo de clasificación al estimar si el precio sube o no sube. 
            Aplica a la señal direccional, no al nivel exacto del precio.
...
def mostrar_definiciones_precios():
    """Muestra definiciones de los componentes de precio de compras."""
...
def mostrar_limitaciones():
    """Muestra advertencias metodológicas para evitar sobreinterpretación."""
    st.markdown(
        """
...
if ejecutar:
    ...
    with tab1:
        ...
        with st.expander("Definiciones técnicas de la predicción EIA"):
            st.markdown(
                """
                **Predicción:** clasificación generada por el modelo sobre la dirección esperada del precio internacional: sube o no sube.

                **Probabilidad de subida:** confianza relativa del clasificador respecto a la clase “Sube”. No debe interpretarse como magnitud del aumento.

                **Accuracy test temporal:** porcentaje de aciertos observado en el periodo de prueba respetando el orden temporal de la serie.

                **Validación cruzada:** evaluación promedio del modelo en distintas particiones de entrenamiento, útil para revisar estabilidad.
                """
            )
    ...
    with tab2:
        ...
        if error_recope:
            ...
            if not base_prod.empty:
                ...
                mostrar_definiciones_precios()

                st.markdown(
                    """
                    **Cómo interpretar este gráfico:**  
                    La serie muestra el comportamiento histórico del precio real de importación para el producto seleccionado. 
                    Sirve como base para observar ciclos, shocks internacionales y cambios en el costo de adquisición.
                    """
                )
    ...
    with tab3:
        ...
        if backtest.empty:
            ...
            c_corr1.metric("Correlación histórica", f"{corr:,.2f}" if not pd.isna(corr) else "Sin dato")
            c_corr2.metric("Relación observada", clasificar_correlacion(corr))

            st.info(texto_transmision_precios(backtest, etiqueta_eia))
            ...
            st.markdown(
                ...
                **Lectura económica:**  
                Cuando ambas curvas se mueven en la misma dirección, se observa evidencia visual de transmisión del componente internacional hacia el costo real de importación. 
                La diferencia de niveles puede deberse a fletes, seguros, rezagos, condiciones comerciales, spreads regionales y composición de los embarques.
                """
            )
    ...
    with tab5:
        ...
        st.markdown(
            ...
            1. **Predicción EIA:** muestra la señal direccional del modelo internacional.
            2. **Compras Recope:** resume la base real de importaciones por producto.
            3. **Comparación:** contrasta el proxy internacional con el precio real observado.
            4. **Análisis técnico:** interpreta errores, cercanía, correlación y desempeño.
            5. **Histórico:** conserva datos y salidas para revisión.

            ### Concepto central

            El objetivo no es afirmar que el proxy EIA replica exactamente una compra específica, sino evaluar si el componente externo captura la tendencia general del costo real de importación.
            """
        )
    ...
    st.markdown(
        ...
        La lectura recomendada es tratarla como un observatorio experimental: primero se revisa la señal internacional, luego las compras reales, después la comparación y finalmente las métricas técnicas.
